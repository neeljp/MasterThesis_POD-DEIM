...
Y_hd = np.zeros((52749,3000))
for i in range(3000):
    Y_hd[:,i] = io.read_PETSc_vec('simulation/full_trajectory_+10/sp' + str(i).zfill(4) + 'ts2879N.petsc')
...
hdnorm= np.linalg.norm(Y_hd,axis=0)
data7 = np.linalg.norm(Y_hd - Y_red,axis=0)/hdnorm
...
Y_hd = np.zeros((52749,3000))
for i in range(3000):
    Y_hd[:,i] = io.read_PETSc_vec('simulation/full_trajectory_+20/sp' + str(i).zfill(4) + 'ts2879N.petsc')
...
hdnorm= np.linalg.norm(Y_hd,axis=0)
data8 = np.linalg.norm(Y_hd - Y_red,axis=0)/hdnorm
...
plt.plot(data5,color='gray',linestyle='--',linewidth=3,marker='>',markevery=markers_on,markersize=10)
plt.plot(data6,color='gray',linestyle='--',linewidth=3,marker='>',markevery=markers_on,markersize=10)
plt.plot(data7,color='gray',linestyle='--',linewidth=3,marker='>',markevery=markers_on,markersize=10)
plt.plot(data8,color='gray',linestyle='--',linewidth=3,marker='>',markevery=markers_on,markersize=10)

labels = [r"$u_r+5\%$",r"$u_r+10\%$",r"$u_r+20\%$",r"$u_r+50\%$",r"FOM $u_r$"]
...
plt.savefig('error_norm_one_base_+50.png', bbox_inches='tight')
plt.show()